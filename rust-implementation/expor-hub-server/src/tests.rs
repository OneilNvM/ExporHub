...
fn should_create_like() {
    ...
    let like = insert_like(&mut conn, 0);
...
fn should_create_dislike() {
    ...
    let dislike = insert_dislike(&mut conn, 0);
...
#[test]
#[should_panic]
fn should_delete_image_by_file_path() {
    use crate::db::db_actions::deletes::delete_image_by_file_path;

    let mut conn = establish_connection();

    let result = delete_image_by_file_path(&mut conn, "path/to/image");
