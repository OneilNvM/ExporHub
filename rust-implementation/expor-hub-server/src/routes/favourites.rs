...
use crate::{
    db::db_actions::{
        deletes::delete_favourite_by_ids,
        inserts::insert_favourite,
        selects::{find_favourite_by_ids, find_favourites_by_user_id, find_project_by_id},
        updates::update_project,
    },
    routes::{ServerResponse, UserAndProjectId, UserId},
    DbPool,
};

#[get("/user-id")]
pub async fn get_favourites_by_user_id(
...
#[post("/unfavourite")]
pub async fn unfavourite_project(
    pool: web::Data<DbPool>,
    ids: web::Query<UserAndProjectId>,
) -> Result<HttpResponse> {
...
#[get("/u-p-id")]
pub async fn get_favourite_by_ids(
    pool: web::Data<DbPool>,
    ids: web::Query<UserAndProjectId>,
) -> Result<HttpResponse> {
    let ids = ids.into_inner();
...
#[post("/new")]
pub async fn create_new_favourite(
    pool: web::Data<DbPool>,
    ids: web::Query<UserAndProjectId>,
) -> Result<HttpResponse> {
    let ids = ids.into_inner();
