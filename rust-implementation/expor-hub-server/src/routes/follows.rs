...
use crate::{db::db_actions::{deletes::delete_follow_by_ids, inserts::insert_follow, selects::{find_follow_by_ids, find_user_by_id, find_user_followings}, updates::update_user}, routes::{ServerResponse, UserId}, DbPool};
...
#[post("/new")]
pub async fn new_follow(pool: web::Data<DbPool>, ids: web::Query<FollowIds>) -> Result<HttpResponse> {
    let ids = ids.into_inner();
    ...
    match follow {
        ...
        Err(_) => Ok(HttpResponse::Ok().json(ServerResponse {
            code: 1, message: "Failed to follow".to_owned()
        }))
    }
}

#[get("/unique-follow")]
pub async fn get_follow_by_ids(pool: web::Data<DbPool>, ids: web::Query<FollowIds>) -> Result<HttpResponse> {
    let ids = ids.into_inner();
    ...
    match follow {
        ...
        Err(_) => Ok(HttpResponse::Ok().json(ServerResponse {
            code: 1, message: "Failed to find follow".to_owned()
        }))
    }
}